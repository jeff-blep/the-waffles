-- Add shipping method to waffles listing.
-- 'ship'   = Chef ships the item
-- 'meetup' = Local handoff, details via messaging
-- 'ffl'    = Firearms only, transfer through licensed FFL dealer

alter table public.waffles
  add column shipping_method text not null default 'ship'
  check (shipping_method in ('ship', 'meetup', 'ffl'));

-- Drop all previous overloads so the new signature is unambiguous.
drop function if exists public.create_waffle(text,text,text,integer,integer,boolean,draw_style,draw_type,timestamptz,jsonb);
...
begin
  ...
  if p_shipping_method not in ('ship', 'meetup', 'ffl') then
