-- Atomic waffle creation: inserts waffle + item + seats in one transaction.
-- Photos are uploaded by the client after this RPC returns, then the client
-- updates waffle_items.photo_urls. If anything fails post-RPC the client
-- deletes the waffle (cascades to items + seats).
...
begin
  ...
  v_platform_fee := (v_total_pot * 10) / 100;
  ...
  insert into public.waffles (
    ...
    'active', v_total_pot, v_platform_fee
